...
class RobotanikRowFSM(Node):
    def __init__(self):
        ...
        # her sıra 1 m genişlikte
        #
        # Koridor merkezleri:
        # 0.55, 2.50, 4.50, 6.50, 8.50, 10.45
        #
        # Robot spawn:
        # x=2.5, y=1.0
        #
        # Bu yüzden rota x=2.50 koridorundan başlatıldı.
        # =====================================================================
        ...
        self.y_bottom_turn = 1.00
        self.y_top_turn = 49.00
        ...
        self.get_logger().info("Bu FSM GitHub DWB YAML ile uyumludur.")
    ...
    def generate_serpentine_waypoints(self):
        ...
        for i, x in enumerate(self.corridor_centers):
            ...
            if going_up:
                # Koridora alttan hizalan
                waypoints.append((x, y_bottom_turn, 1.5708))
                waypoints.append((x, y_bottom_entry, 1.5708))

                # Koridor içinde düz ilerle
                waypoints.append((x, y_top_entry, 1.5708))

                # Koridordan üst dönüş alanına çık
                waypoints.append((x, y_top_turn, 0.0))

                # Sonraki koridora üstten yatay geç
                if i < len(self.corridor_centers) - 1:
                    next_x = self.corridor_centers[i + 1]
                    waypoints.append((next_x, y_top_turn, 0.0))

                    # Yeni koridora girmeden önce aşağı yöne hizalan
                    waypoints.append((next_x, y_top_turn, -1.5708))
                    waypoints.append((next_x, y_top_entry, -1.5708))

            else:
                # Koridora üstten hizalan
                waypoints.append((x, y_top_turn, -1.5708))
                waypoints.append((x, y_top_entry, -1.5708))

                # Koridor içinde düz ilerle
                waypoints.append((x, y_bottom_entry, -1.5708))

                # Koridordan alt dönüş alanına çık
                waypoints.append((x, y_bottom_turn, 0.0))

                # Sonraki koridora alttan yatay geç
                if i < len(self.corridor_centers) - 1:
                    next_x = self.corridor_centers[i + 1]
                    waypoints.append((next_x, y_bottom_turn, 0.0))

                    # Yeni koridora girmeden önce yukarı yöne hizalan
                    waypoints.append((next_x, y_bottom_turn, 1.5708))
                    waypoints.append((next_x, y_bottom_entry, 1.5708))
    ...
    def request_cancel_goal(self):
        if self.goal_handle is None:
            self.get_logger().warn("İptal edilecek aktif goal yok.")
    ...
    def loop(self):
        ...
        if self.state == MissionState.SEND_GOAL:
            ...
            # Goal response callback state'i WAIT_RESULT yapacak.
            self.state = MissionState.STARTUP_WAIT
            self.startup_time = time.time()
        ...
        if self.state == MissionState.WAIT_RESULT:
            ...
            if self.obstacle_counter >= self.obstacle_confirm_count:
                self.get_logger().warn("Ön engel 5 ölçüm doğrulandı. Goal iptal edilecek.")
                self.obstacle_counter = 0
                self.publish_stop()
