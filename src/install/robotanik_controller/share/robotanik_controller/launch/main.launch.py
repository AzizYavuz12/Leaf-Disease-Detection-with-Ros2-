...
def generate_launch_description():
    ...
    # main_controller.py'yi hangi pakete koyduysan onun adını aşağıya yazacaksın:
    controller_pkg_name = 'robotanik_vision' 
    ...
    vision_launch = TimerAction(
        ...
        actions=[IncludeLaunchDescription(
            PythonLaunchDescriptionSource(
                os.path.join(vision_pkg_dir, 'launch', 'vision.launch.py')
            )
        )]
    )

    # 4. Main Controller (Sistem Beyni) Düğümü
    # Tüm sistem ayağa kalktıktan sonra, 8. saniyede devreye girer
    main_controller_node = TimerAction(
        period=8.0,
        actions=[Node(
            package=controller_pkg_name,     # main_controller.py hangi paketteyse
            executable='main_controller',    # setup.py'deki 'console_scripts' adı
